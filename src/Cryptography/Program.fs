...
let main argv =
    ...
    printfn $"Discriminant: {discriminant a b p}"
    Console.WriteLine()
    
    let yPairs = List.map (fun y -> (y, leftPart y p)) numbers
    printfn $"(y, y^2): {showList (yPairs.ToList())}"
    Console.WriteLine()
    
    let xPairs = List.map (fun x -> (x, rightPart x a b p)) numbers
    printfn $"(x, x^3 + {a}x + {b}): {showList (xPairs.ToList())}"
    Console.WriteLine()
    
    let pairs = getPairs xPairs yPairs
    let curve = Curve(a, b, p)
    let points = getPoints pairs (curve)
    printfn $"Points: {showList points}"
    Console.WriteLine()
    
    printfn "Table of sums:"
    getTable points
    Console.WriteLine()
    
    match getCreativeElement points with
    | Some(creativeElement) -> printfn $"{getCyclicProof creativeElement points}"
    | None -> printfn "Group is not cyclic"
    Console.WriteLine()
